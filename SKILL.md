...
### Step 1.1: Querying Positions (READ)

To show the user their current positions:
1. Call `GET /contract/private/position-v2` (omit `symbol` for all, or add `?symbol=ETHUSDT` for one)
2. Auth: KEYED — only `X-BM-KEY` header needed, no signature
3. Filter rows where `current_amount != "0"` — these are active positions
4. Display: symbol, position_side, current_amount, entry_price, leverage, open_type, liquidation_price, unrealized_pnl
...
|---|---|---|---|
| Buy, market | `buy` | `market` | symbol, side, type, `notional` (spend amount in quote currency, e.g. USDT) |
| Buy, limit | `buy` | `limit` | symbol, side, type, price, `size` (base currency qty, e.g. BTC) |
| Sell, market | `sell` | `market` | symbol, side, type, `size` (base currency qty) |
| Sell, limit | `sell` | `limit` | symbol, side, type, price, `size` (base currency qty) |

> **`notional` vs `size`:** Market buy uses `notional` (how much quote to spend). All other types use `size` (how much base to trade).

### Step 2.5: Spot ↔ Futures Transfer

**This uses the FUTURES base URL, not the spot base URL.**

| Direction | `type` value |
|---|---|
| Spot → Futures | `"spot_to_contract"` |
| Futures → Spot | `"contract_to_spot"` |

**Endpoint:** `POST https://api-cloud-v2.bitmart.com/account/v1/transfer-contract`
- Currency: USDT only
- Rate limit: 1 req/2sec

```json
{"currency":"USDT","amount":"50","type":"spot_to_contract"}
```

> ⚠️ Do NOT use `/spot/v1/transfer-contract` — that path does not exist.
