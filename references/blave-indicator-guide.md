...
Blave 平台綁定帳戶中，資產規模前 10% 交易員的合約淨曝險。這是**全市場單一序列**：把這些帳戶持有的所有幣種永續部位加總，沒有 symbol 維度——圖上疊的價格線只是對照用，換 symbol 只換價格線，曝險值完全相同。
...
篩選方式：綁定帳戶依資產（錢包餘額加總）排序取前 10%，再排除資產 ≤ $1,000 USD 的帳戶；只看資產規模，不看績效。

數值是原始比率：各交易員（淨名目部位 ÷ 自身總資產）的中位數 ×100，不是 z-score——±1 / ±2 這類 z-score 門檻不適用。
